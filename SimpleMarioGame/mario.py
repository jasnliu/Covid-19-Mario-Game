...
class Dragon:
    ...
    def life(self):
        return self.dragonlife
    def hitdragon(self):
        self.dragonlife=self.dragonlife-1
...
class Flames:
    ...
    def __init__(self):
        self.flames = pygame.image.load('covid.png')
        self.flames_img = pygame.transform.scale(self.flames, (50, 50))
...
class Bullet:
    ...
    def __init__(self):
        self.bullet = pygame.image.load('mask.png')
        self.bullet_img = pygame.transform.scale(self.bullet, (90, 60))
        self.bullet_img_rect = self.bullet_img.get_rect()
        self.bullet_img_rect.left = mario.mario_img_rect.right
        self.bullet_img_rect.top = mario.mario_img_rect.top + 30

    def update(self):
        canvas.blit(self.bullet_img, self.bullet_img_rect)

        if self.bullet_img_rect.left < WINDOW_WIDTH:
            self.bullet_img_rect.left += self.bullet_velocity
...
class Mario:
    ...
    def update(self):
        ...
        if self.mario_img_rect.top <= cactus_img_rect.bottom and False:
            game_over()
            if SCORE > self.mario_score:
                self.mario_score = SCORE
        if self.mario_img_rect.bottom >= fire_img_rect.top and False:
            game_over()
            if SCORE > self.mario_score:
                self.mario_score = SCORE


        if self.up and self.mario_img_rect.top>cactus_img_rect.bottom:
            self.mario_img_rect.top -= 10
        if self.down and self.mario_img_rect.top<fire_img_rect.top:
    ...
    while True:
        ...
        while True:
            ...
            for b in bullet_list:
                if b.bullet_img_rect.right >= WINDOW_WIDTH:
            ...
            score_font = font.render('Vaccine:'+str(SCORE), True, GREEN)
            ...
            top_score_font = font.render('Covid-19 Cases:'+str(dragon.life()),True,GREEN)
            ...
            mario.update()
            for b in bullet_list:
                if b.bullet_img_rect.colliderect(dragon.dragon_img_rect):
                    dragon.hitdragon()
            for f in flames_list:
                if f.flames_img_rect.colliderect(mario.mario_img_rect): # and False:
                    game_over()
